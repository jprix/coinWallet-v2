...
<script>
    ...
    onMount( async () => {
        try {
            const getBalances = await fetch(`/api/v3/balances?access_token=${access_token}`,{
                method: "GET"
        })
    ...
    console.log(userBalances)
        _.each(userBalances, item => item.available_balance = parseInt(item.available_balance.value, 13));
